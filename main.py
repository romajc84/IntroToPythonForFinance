...
while stock != "quit":
    ...
    for x in smasUsed:  # This for loop calculates the SMAs for the stated periods and appends to dataframe
        ...
        prices['SMA_' + str(sma)] = prices.iloc[:,
                                                4].rolling(window=sma).mean()

    # calculate Bollinger Bands
    BBperiod = 15  # choose moving avera
    stdev = 2
    # calculates sma and creates a column in the dataframe
    prices['SMA' + str(BBperiod)] = prices.iloc[:,
                                                4].rolling(window=BBperiod).mean()
    ...
    prices['LowerBand'] = prices['SMA' +
                                 str(BBperiod)] - (stdev * prices['STDEV'])
    prices['UpperBand'] = prices['SMA' +
                                 str(BBperiod)] + (stdev * prices['STDEV'])  # calculates upper band
    ...
    prices["RolHigh"] = prices["High"].rolling(
        window=Period).max()  # Finds high of period
    prices["RolLow"] = prices["Low"].rolling(
        window=Period).min()  # finds low of period
    ...
    for index in range(len(pivots)):  # Iterates through pivot array
        ...
        plt.plot_date([dates[index] - (timeD * .075), dates[index] + timeD],  # Plots horizontal line at pivot value
                      ...
                      marker=',')
        plt.annotate(
            str(pivots[index]), (mdates.date2num(dates[index]), pivots[index]),
            xytext=(-10, 7),
